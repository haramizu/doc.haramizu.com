...
description: このドキュメントでは、Sitecore CDP + Personalize の機能を検証するためのツール、Tampermonkey のインストールおよび動作確認まで進めていきます。
sidebar:
  order: 10
...
Sitecore Personalize は、クライアント側の JavaScript を利用してウェブページのパーソナライズを実現するツールです。これにより、ユーザーの行動や属性に基づいて、個別にカスタマイズされたコンテンツを表示することができます。

本ドキュメントでは、まだ Sitecore Personalize のタグを埋め込んでいないウェブサイトに対して、ブラウザの拡張機能を利用してパーソナライズのテストを行う方法を紹介します。この方法を使用することで、開発中のサイトや非本番環境でのパーソナライズの動作確認が容易になります。
...
本ドキュメントでは、Google Chrome もしくは Microsoft Edge でこの機能拡張をインストール、動作検証を進めていきます。
