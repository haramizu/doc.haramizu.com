...
2. Copy the file `headapps/nextjs-starter/src/assets/app.css` and create `headapps/tailwind-starter/src/styles/app.css`.

3. Create the file `headapps/tailwind-starter/src/styles/globals.css` and set the following styles.
...
4. Change the code of `headapps/tailwind-starter/src/pages/_app.tsx` that loads the stylesheet as follows.
...
5. When loading the stylesheet above, the file is specified using `@/`. To enable this, add the following line to the Typescript settings tsconfig.json.
...
1. Delete the folder `headapps/tailwind-starter/src/assets` where the stylesheet-related files are located.
...
To arrange them horizontally, add the flex class. Create a file `headapps/tailwind-starter/src/styles/components/ColumnSplitter.css` and apply the following code.
...
```css
// headapps/tailwind-starter/src/styles/components/RichText.css
.component.rich-text {
  h1 {
    @apply text-4xl;
  }
...
After importing the above styles in the `headapps/tailwind-starter/src/styles/globals.css` file, you can preview the changes and see that the display has been updated.
...
To enable this and change the list display, add the following code to `headapps/tailwind-starter/src/styles/components/RichText.css`:
...
```css
// headapps/tailwind-starter/src/styles/components/Container.css
...
Add the above file to `headapps/tailwind-starter/src/styles/globals.css` to enable the container background colors. Below is the result of placing a container and specifying the background color.
...
```css
// headapps/tailwind-starter/src/styles/components/LinkList.css
.component.link-list {
  ul {
    @apply list-disc pb-2 pl-3 pt-2;
  }
...
Add the above file to `headapps/tailwind-starter/src/styles/globals.css` to display the link list as shown below.
...
```css
// headapps/tailwind-starter/src/styles/components/Navigation.css
