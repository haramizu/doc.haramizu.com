...
- [CSS とスタイリング - Tailwind CSS](https://starlight.astro.build/ja/guides/css-and-tailwind/#tailwind-css)
...
ウィザードが起動して、作成するプロジェクトの設定を確認してきます。今回は以下のように設定をしていきました。
...
![npm create astro](./assets/starlight01.png)

設定が完了すると、以下のようなメッセージが表示されます。

![Good luck out there](./assets/starlight02.png)

プロジェクトが完成しました。画面に表示されているように、パスに移動をして起動すると、サンプルのコンテンツが含まれた形で立ち上がっているのが分かります。

![docs with tailwind](./assets/starlight03.png)
...
CLI が利用できるようになっている場合は、プロジェクトのトップで以下のコマンドでログインをします。まず最初にどのアカウントでログインをするかの確認画面が表示されます。
...
私の場合は GitHub を利用しているため、ブラウザが起動してログインが完了することで、Vercel CLI でのログインが完了します。

続いて Vercel のプロジェクトとリンクをします。
...
![Congratulations](./assets/starlight06.png)

## Sitemap / robots.txt の作成

作成したサイトのサイトマップを利用できるように、Sitemap の機能を有効にします。この設定は以下のページに紹介されています。

- [Starlightのカスタマイズ - サイトマップを有効化する](https://starlight.astro.build/ja/guides/customization/#サイトマップを有効化する)
...
ここでは Google Tag Manager の実装方法を案内します。これはサイト全体で利用する設定となるため、 `astro.config.mjs` に head を利用して記述する形となります。まず最初に、Google Tag Manager の値を環境変数で管理することができるように、`.env.local` のファイルを作成して以下の定義を追加します。
...
続いて設定ファイルを更新します。環境変数を読み込んで、タグに割り当てています。
